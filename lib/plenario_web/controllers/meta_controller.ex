defmodule PlenarioWeb.MetaController do
  ...
  def ingest_dataset(conn, %{"slug" => slug}) do
    meta = MetaActions.get(slug, with_user: true)
    PlenarioEtl.Worker.async_load!(meta.id)
  ...
  def do_update_description(conn, %{
        "slug" => slug,
        "meta" => %{"description" => description, "attribution" => attribution}
      }) do
    meta = MetaActions.get(slug, with_user: true)
  ...
  def do_update_source_info(conn, %{
        "slug" => slug,
  ...
  def get_update_refresh_info(conn, %{"slug" => slug}) do
    ...
    render(
      conn,
      "update_refresh_info.html",
      changeset: changeset,
      action: action,
      rr_choices: rr_choices
    )
  end

  def do_update_refresh_info(conn, %{
        "slug" => slug,
        "meta" => %{
    ...
    Logger.info(
      "Updating refresh rate: #{refresh_rate}, refresh interval: #{refresh_interval}, refresh start: #{
        refresh_starts_on
