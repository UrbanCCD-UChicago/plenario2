defmodule Plenario.Actions.MetaActionsTest do
  ...
  describe "get_agg_data" do
    test "when meta has numerical fields", %{user: user} do
      {:ok, meta} = MetaActions.create("num fields", user, "https://example.com/n", "csv")
      {:ok, _} = DataSetFieldActions.create(meta, "Obs 1", "float")
      {:ok, _} = DataSetFieldActions.create(meta, "Obs 2", "float")
      {:ok, _} = DataSetFieldActions.create(meta, "Obs Mean", "float")
      {:ok, _} = DataSetFieldActions.create(meta, "Obs Date", "timestamp")
      :ok = DataSetActions.up!(meta)
      insert =
        """
        INSERT INTO "#{meta.table_name}"
          ("Obs 1", "Obs 2", "Obs Mean", "Obs Date")
        VALUES
          (1.1, 1.3, 1.2, '2018-01-01 00:00:00'),
          (1.2, 1.4, 1.3, '2018-02-01 00:00:00'),
          (1.3, 1.5, 1.4, '2018-03-01 00:00:00'),
          (1.4, 1.6, 1.5, '2018-04-01 00:00:00'),
          (1.5, 1.7, 1.6, '2018-05-01 00:00:00'),
          (3.1, 2.5, 2.8, '2018-01-02 00:00:00'),
          (3.2, 2.6, 2.9, '2018-02-02 00:00:00'),
          (3.3, 2.7, 3.0, '2018-03-02 00:00:00'),
          (3.4, 2.8, 3.1, '2018-04-02 00:00:00'),
          (3.5, 2.9, 3.2, '2018-05-02 00:00:00');
        """
      Repo.query!(insert)
      ...
      {:line, labels, data} = MetaActions.get_agg_data(meta)

      assert labels == [
        ~N[2018-01-01 00:00:00],
        ~N[2018-02-01 00:00:00],
        ~N[2018-03-01 00:00:00],
        ~N[2018-04-01 00:00:00],
        ~N[2018-05-01 00:00:00],
      ]

      assert data == [
        {"Obs 1", [2.1, 2.2, 2.3, 2.4, 2.5]},
        {"Obs 2", [1.9, 2.0, 2.1, 2.2, 2.3]},
        {"Obs Mean", [2.0, 2.1, 2.2, 2.3, 2.4000000000000004]}  # wtf postgres
      ]
    end

    test "when meta has boolean fields", %{user: user} do
      {:ok, meta} = MetaActions.create("bool fields", user, "https://example.com/b", "csv")
      {:ok, _} = DataSetFieldActions.create(meta, "Obs", "boolean")
      {:ok, _} = DataSetFieldActions.create(meta, "Obs Date", "timestamp")
      :ok = DataSetActions.up!(meta)
      insert =
        """
        INSERT INTO "#{meta.table_name}"
          ("Obs", "Obs Date")
        VALUES
          (true, '2018-01-01 00:00:00'),
          (true, '2018-01-01 00:00:00'),
          (true, '2018-01-01 00:00:00'),
          (false, '2018-01-01 00:00:00'),
          (false, '2018-01-01 00:00:00'),
          (false, '2018-01-01 00:00:00'),
          (false, '2018-01-01 00:00:00'),
          (false, '2018-01-01 00:00:00'),
          (false, '2018-01-01 00:00:00'),
          (true, '2018-02-01 00:00:00'),
          (true, '2018-02-01 00:00:00'),
          (true, '2018-02-01 00:00:00'),
          (true, '2018-02-01 00:00:00'),
          (true, '2018-02-01 00:00:00'),
          ...
          (false, '2018-02-01 00:00:00'),
          (false, '2018-02-01 00:00:00'),
          (true, '2018-03-01 00:00:00'),
          (true, '2018-03-01 00:00:00'),
          (true, '2018-03-01 00:00:00'),
          (false, '2018-03-01 00:00:00'),
          (false, '2018-03-01 00:00:00'),
          (false, '2018-03-01 00:00:00'),
          (false, '2018-03-01 00:00:00'),
          (false, '2018-03-01 00:00:00'),
    ...
    test "when meta has only virtual point fields", %{user: user} do
      {:ok, meta} = MetaActions.create("loc fields", user, "https://example.com/l", "csv")
      {:ok, lat} = DataSetFieldActions.create(meta, "Lat", "float")
      {:ok, lon} = DataSetFieldActions.create(meta, "Lon", "float")
      {:ok, _} = DataSetFieldActions.create(meta, "Date", "timestamp")
      {:ok, _} = DataSetFieldActions.create(meta, "Notes", "text")
      ...
      insert =
        ...
        INSERT INTO "#{meta.table_name}"
          ("Lat", "Lon", "Date", "Notes")
        VALUES
          (1, 2, '2018-01-01 00:00:00', 'blah blah blah'),
          (2, 3, '2018-01-01 00:00:00', 'blah blah blah'),
          (3, 4, '2018-01-01 00:00:00', 'blah blah blah'),
          (4, 5, '2018-01-01 00:00:00', 'blah blah blah'),
          (5, 6, '2018-01-01 00:00:00', 'blah blah blah');
        """
      Repo.query!(insert)
      ...
      {:map, "Locations", data} = MetaActions.get_agg_data(meta)
